...
for x in range(len(a)+1):
    for y in range(len(b)+1):
        if(x>0 and y>0):
            d = lengths[x][y-1][0] - 5
            e =lengths[x-1][y][0]-5
            f = lengths[x-1][y-1][0]+edges[(a[x-1], b[y-1])]
            troll = max(d, e, f)
            if(troll ==d):
                lengths[x][y] = (troll, "Y")
            elif(troll==e):
            ...
            lengths[x][y] = (lengths[x-1][y][0]-5, "X")
        elif(y>0):
            lengths[x][y] = (lengths[x][y-1][0]-5, "Y")
f = open('globalAlignment.txt', 'w')
f.write(str(lengths[len(a)][len(b)][0])+"\n")
aPath = []
bPath = []
